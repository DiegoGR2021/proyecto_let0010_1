...
ggplot(retornos_capital_a, aes(x = t, y = A)) +
  ...
  ggtitle("Figura 1: Retornos mensuales de AFP Capital Fondo A, período 2008-2021") + xlab("Año") + ylab("Retornos")
```
...
ggAcf(new_rets$A, lag.max = 20) +
  ggtitle("Figura 2: Autocorrelación de los retornos del Fondo A Capital")
```

```{r}
ggPacf(new_rets$A ,lag.max = 20) +
  ggtitle("Figura 3: Autocorrelación parcial de los retornos del Fondo A Capital")
```
